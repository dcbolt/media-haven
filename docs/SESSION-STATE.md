...
- **Persistent Roku overlay (2026-09-10):** occupied + vacant `/tv` signage always shows a bottom banner — “Press ROKU on your remote to begin streaming.” Toggle `TV_ROKU_STREAM_OVERLAY` in `lib/tv-entertainment.ts` (no CMS coach-copy setting existed). Hidden on the detailed Watch TV slide so two coaches do not stack. Pairing / takeover stay clear.
