...
- **Persistent stream overlay (2026-09-10):** occupied + vacant `/tv` signage always shows a bottom banner — “Push home on your remote to start streaming.” (Home-button default; earlier ROKU line is not the visible copy.) Toggle `TV_ROKU_STREAM_OVERLAY` in `lib/tv-entertainment.ts` (no CMS coach-copy setting existed). Hidden on the detailed Watch TV slide so two coaches do not stack. Pairing / takeover stay clear.
- **Signage ambience (2026-09-10):** `/tv` plays procedural ocean + resonant sound-bath beds under occupied and vacant signage. Host CMS mute/volume/bed (`settings.ambience`). `?ambience=off` mutes. Videos stay muted. Sits under the #149 Home overlay. See [`docs/AMBIENCE.md`](./AMBIENCE.md).
