    ...
    <section className="rounded-2xl bg-white p-6 shadow-md">
      ...
      <form onSubmit={subscribe} className="mt-4 space-y-2">
        ...
        <input
          ...
          className="w-full rounded-xl border border-sand-300 p-3 text-lg outline-none focus:border-ocean-500"
        />
        <label className="flex items-start gap-2 text-ocean-900/70">
          <input
            type="checkbox"
            checked={consentMarketing}
            onChange={(e) => setConsentMarketing(e.target.checked)}
            className="mt-1 h-5 w-5 accent-ocean-500"
          />
          <span>
            Also send me occasional offers on future stays at The Florida
            Havens. (Optional — launch alerts work either way.)
          </span>
        </label>
        <button
          type="submit"
